...
class vcard_convert extends Contact_Vcard_Parse
{
	...
	function normalize()
	{
		...
		{
			$vcard = new vCard;
			$vcard->version = isset($card['VERSION'])?(float)$card['VERSION'][0]['value'][0][0]:'';
			...
			// extract birthday
			if(isset($card['BDAY']) && is_array($card['BDAY']))
			{
			...
			// extract job_title
			if (isset($card['TITLE']) && is_array($card['TITLE']))
				$vcard->jobtitle = $card['TITLE'][0]['value'][0][0];

			// extract org and dep
			if (isset($card['ORG']) && is_array($card['ORG']) && ($temp = $card['ORG'][0]['value']))
			{
			...
			{
				$prefix = 'ITEM'.$n;
				if (isset($card["$prefix.TEL"]) && is_array($card["$prefix.TEL"])) {
					$this->parse_tel($card["$prefix.TEL"], $vcard);
				}
				if (isset($card["$prefix.URL"]) && is_array($card["$prefix.URL"])) {
					$this->parse_url($card["$prefix.URL"], $vcard);
				}
				if (isset($card["$prefix.ADR"]) && is_array($card["$prefix.ADR"])) {
					$this->parse_adr($card["$prefix.ADR"], $vcard);
				}
				if (isset($card["$prefix.X-ABADR"]) && is_array($card["$prefix.X-ABADR"])) {
					$this->parse_cc($card["$prefix.X-ABADR"], $vcard);
				}
				if (isset($card["$prefix.X-ABRELATEDNAMES"]) && is_array($card["$prefix.X-ABRELATEDNAMES"])) {
			...
			if ($n < 2) { //as only 3 e-mail address will be exported we don't need to search for more
				for ($n = 1; $n <= 9; $n++) {
					if (isset($card["ITEM$n.EMAIL"]) && is_array($card["ITEM$n.EMAIL"]))
					{
			...
			if (isset($card['X-ICQ']) && is_array($card['X-IQC']))
			...
			if (isset($card['PHOTO']) && is_array($card['PHOTO'][0]))
				$vcard->photo = array('data' => $card['PHOTO'][0]['value'][0][0], 'encoding' => $card['PHOTO'][0]['param']['ENCODING'][0]);

			$vcard->categories = isset($card['CATEGORIES'])?join(',', (array)$card['CATEGORIES'][0]['value'][0]):'';
